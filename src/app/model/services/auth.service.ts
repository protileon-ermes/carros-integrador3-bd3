...
export class AuthService {
  ...
  constructor(private firebase: FirebaseService, private fireAuth: AngularFireAuth, private router: Router, private ngZone: NgZone
    ...
    this.fireAuth.authState.subscribe(user =>{
      if(user){
        ...
        localStorage.setItem('user', JSON.stringify(this.usuarioDados));//localStorage: guarda localmente
      }
      else{
  ...
  public signIn(email: string, senha: string){
    return this.fireAuth.signInWithEmailAndPassword(email,senha);
  }

  public signUpWithEmailPassword(email:string, senha:string){
